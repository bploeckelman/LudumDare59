...
void main() {
    ...
    float lineEdge = .08;
    float topLine = cubicPulse(.95, lineEdge, v_texCoords.y);
    float bottomLine = cubicPulse(.05, lineEdge, v_texCoords.y);
    vec4 lineColor = v_color;
    lineColor.rgb = mix(v_color.rgb, u_edgeColor.rgb, topLine);
    lineColor.rgb = mix(lineColor.rgb, u_edgeColor.rgb, bottomLine);
    vec4 finalColor = lineColor + vec4(-.2 + vec3(n1+n2+n3)*.7, 0);
