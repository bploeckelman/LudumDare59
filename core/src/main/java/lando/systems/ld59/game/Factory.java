...
public class Factory {
    ...
    public static Entity enemyShip(EnemyTag.EnemyType enemyType, float posX, float posY, float velX, float velY) {
        var animEnemy = AnimEnemy.of(enemyType);
        // NOTE: all AnimEnemy ship types have the same size / dimensions 80x80
        var size = animEnemy.get().getKeyFrame(0f).getRegionWidth();
        return enemyShip(enemyType, posX, posY, velX, velY, size);
    }

    public static Entity enemyShip(EnemyTag.EnemyType enemyType, float posX, float posY, float velX, float velY, float size) {
        var animEnemy = AnimEnemy.of(enemyType);
        var entity = createEntity();
        var energyColorType = EnergyColor.of(enemyType);
        var position = new Position(posX, posY);
        var animator = new Animator(animEnemy, new Vector2(size, size), new Vector2(size / 2f, size / 2f));
        ...
        enemyTag.randomOffset = MathUtils.random(0f, 1.0f);
